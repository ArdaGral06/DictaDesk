"""Single-namespace implementation for the desk_platform package.

This is the source of truth for all desktop automation helpers. Everything
lives in one module so the many cross-cutting references between the former
common/automation/gui/files sections resolve without circular imports. The
public package (desk_platform/__init__.py), the submodule shims
(common/automation/gui/files.py) and platform_actions.py all re-export from
here. Edit this file directly. The `# ===== <name> =====` markers below just
indicate the original section each block came from.
"""
...
def start_app_verified(
    ...
    prefer_shortcut = (
        candidate
        and not _looks_like_path(raw_value)
        and PREFER_START_MENU_SHORTCUT
    )

    methods: list[tuple[str, callable]] = []
    if prefer_shortcut:
        methods.append(("shortcut", lambda c=candidate: os.startfile(str(c))))
    ...
    if has_config_path and not prefer_shortcut:
        methods.append(("exe", lambda: os.startfile(resolved)))
    elif has_config_path:
        methods.append(("exe_fallback", lambda: os.startfile(resolved)))

    if candidate and not prefer_shortcut:
        methods.append(("shortcut", lambda c=candidate: os.startfile(str(c))))
...
DISCORD_CHANNEL_ALIASES = {
    "genel": ["genel", "general", "genel-sohbet", "genel sohbet"],
    "genel-sohbet": ["genel-sohbet", "genel sohbet", "genel", "general"],
    "genel sohbet": ["genel sohbet", "genel-sohbet", "genel", "general"],
    "general": ["general", "genel", "genel-sohbet", "genel sohbet"],
    "duyurular": ["duyurular", "announcements"],
    "announcements": ["announcements", "duyurular"],
}


def _discord_name_variants(name: str) -> list[str]:
    name = (name or "").strip()
    if not name:
        return []
    plain = name.lstrip("#")
    items = [plain, f"#{plain}"]
    folded = fold_text(plain)
    for alias in DISCORD_CHANNEL_ALIASES.get(folded, []):
        items.append(alias)
        items.append(f"#{alias.lstrip('#')}")
    seen: set[str] = set()
    out: list[str] = []
    for item in items:
        key = fold_text(item)
...
def _discord_dismiss_overlays():
    send_hotkey("escape")
    time.sleep(0.15)


def _discord_quick_switcher_select(query: str):
    query = (query or "").strip()
    if not query:
        return False
    _discord_dismiss_overlays()
    send_hotkey("ctrl k")
    time.sleep(0.65)
    type_text(query)
    time.sleep(1.2)
...
def _discord_click_in_sidebar(target: str, ui_lang: str | None = None) -> bool:
    ...
    if not rect:
        try:
            gui_click_text(target, ui_lang=ui_lang)
            return True
        except Exception:
            return False
    ...
    sidebar_rect = (left, top, left + int(width * 0.42), bottom)
    for _ in range(3):
        ...
        try:
            ...
            screen_size = None
        image_path, offset = _crop_image(full_path, sidebar_rect)
        lang = OCR_LANG_TR if ui_lang == "tr" else OCR_LANG_EN
        try:
            words = _ocr_words_from_image(
                image_path, lang=OCR_LANG_BOTH, offset=offset, min_conf=35
            )
        except Exception:
            words = _ocr_words_from_image(
                image_path, lang=lang, offset=offset, min_conf=35
            )
        box = _find_text_box(
            words, target, container_rect=sidebar_rect, screen_size=screen_size
        )
        if box:
            x = int((box[0] + box[2]) / 2)
            y = int((box[1] + box[3]) / 2)
...
def _discord_select_server(server: str, ui_lang: str | None = None) -> bool:
...
def _discord_select_channel(channel: str, ui_lang: str | None = None) -> bool:
    ...
    for name in _discord_name_variants(channel):
        if _discord_click_in_sidebar(name, ui_lang=ui_lang):
            time.sleep(0.7)
            return True

    # 2) Quick switcher with #channel (server must already be selected).
    for name in _discord_name_variants(channel):
        plain = name.lstrip("#")
        _discord_quick_switcher_select(f"#{plain}")
        time.sleep(1.0)
        if _discord_click_in_sidebar(plain, ui_lang=ui_lang):
            return True
        return True
    return False


def navigate_discord_server_channel(
    server: str | None = None,
    channel: str | None = None,
    ui_lang: str | None = None,
) -> bool:
    """Open the named Discord server, then the text channel inside it."""
    server = (server or "").strip()
    channel = (channel or "").strip().lstrip("#")
    if not server and not channel:
        return False
    if not ensure_app_focus("discord", ui_lang=ui_lang):
        return False
    time.sleep(0.7)
    if server:
        if not _discord_select_server(server, ui_lang=ui_lang):
            return False
        time.sleep(1.2)
    if channel:
        if not _discord_select_channel(channel, ui_lang=ui_lang):
            return False
        time.sleep(0.7)
    return True


def discord_post_in_server_channel(
    server: str | None,
    channel: str | None,
    message: str,
    ui_lang: str | None = None,
    ...
    if not message:
        return False
    if not navigate_discord_server_channel(server, channel, ui_lang=ui_lang):
        return False
    time.sleep(0.5)
    type_text(message)
    time.sleep(0.12)
...
def screen_contains_text(target: str, ui_lang: str | None = None) -> bool:
    """Quick OCR/UIA check that text is visible on screen."""
    ...
    try:
        ...
        if uia_wait_text(needle, timeout_sec=0.8):
            return True
    except Exception:
        pass
    try:
        ...
        try:
            ...
            words = _ocr_words_from_image(image_path, lang=lang_primary, offset=offset, min_conf=40)
        return _find_text_box(words, needle, container_rect=rect, screen_size=screen_size) is not None
    except Exception:
        return False
...
# Export everything (including single-underscore helpers) so the thin
# submodule shims and the package facade can re-export them.
